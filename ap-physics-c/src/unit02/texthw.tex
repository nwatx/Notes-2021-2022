...
% 8. A plane flies 483 km east from city $A$ to city in 45.0 min and then 966 km
% south from city $B$ to city $C$ in 150$h$.
...
$$t = \pm 3.0305$$, so the projectile must remain \boxed{$3.0305$} seconds in the air.
...
\noindent 28. In Fig. 4-34, a stone is projected at a cliff of height h with an initial speed of 42.0 m/s directed
...
\noindent 29. A projectile's launch speed is five times its speed at maximum height.
...
% \noindent Since maximum height is when $v = v_{y0} + at = 0$,
% then $0 = v_{y0} - 9.8t$, so $v_{y0} = 9.8t$. Furthermore, the original launch speed
% is given by the equation $s_0 = \sqrt{{v_{x0}}^2 + {v_{y0}}^2}$. The final launch
% speed is given by the equation $s_m = v_x$.

% $$5v_x = s_0$$

% $$5v_0cos(\theta) = \sqrt{v_0cos(\theta)^2 + v_0sin(\theta)^2}$$

% We can relate $v$ with $v_0$ with $v_x = v_0cos(\theta)$


% the below problem is done completely incorrectyl
\noindent 39. n Fig. 4-37, a ball is thrown leftward from the left edge of the
